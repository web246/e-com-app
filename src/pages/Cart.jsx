...
const SHIP_FEE = 150;
const VALID_COUPON = 'LINET10';

export default function Cart() {
  const { items, removeItem, updateQuantity, subtotal, coupon, setCoupon } = useCart();
      ...
      <div className="min-h-screen bg-[#EFF6FF]">
        ...
        <div className="max-w-xl mx-auto px-4 pt-32 pb-32 text-center">
          <div className="w-24 h-24 rounded-full bg-blue-50 flex items-center justify-center mx-auto mb-6">
            <ShoppingBag size={40} className="text-[#005BB5]" />
          </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-3">
            {items.map(({ key, product, quantity }) => (
              ...
              >
                <Link to={`/product/${product.id}`} className="w-20 h-20 rounded-xl overflow-hidden flex-shrink-0 bg-blue-50">
                ...
                <div className="flex flex-col items-end gap-2">
                  <button onClick={() => removeItem(key)} className="text-slate-300 hover:text-red-500">
                    <Trash2 size={16} />
                  </button>
                  <div className="flex items-center border border-slate-200 rounded-lg">
                    <button onClick={() => updateQuantity(key, quantity - 1)} className="p-1.5"><Minus size={12} /></button>
                    <span className="w-7 text-center text-xs font-semibold">{quantity}</span>
                    <button onClick={() => updateQuantity(key, quantity + 1)} className="p-1.5"><Plus size={12} /></button>
                  </div>
                </div>
              </motion.div>
            ))}
            ...
            <div className="flex gap-2 mb-4">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  placeholder="Coupon code (try LINET10)"
                  className="w-full pl-8 pr-2 py-2 text-sm border border-slate-200 rounded-xl outline-none focus:border-[#005BB5]"
                />
              </div>
              <button onClick={applyCoupon} className="px-4 py-2 text-sm font-semibold text-[#005BB5] border border-[#005BB5]/30 rounded-xl hover:bg-blue-50">Apply</button>
            </div>
