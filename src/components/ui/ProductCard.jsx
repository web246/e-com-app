...
export default function ProductCard({ product, compact = false }) {
  ...
  const cardClass = compact
    ? 'rounded-[8px] border border-[#D9B48A]/60 bg-white p-[1px] shadow-[0_0_0_1px_rgba(139,94,59,0.08),0_2px_5px_rgba(74,42,26,0.06)] w-full'
    : 'rounded-2xl border border-[#D9D2CB] bg-white shadow-sm';
  const imageClass = compact ? 'aspect-[4/5]' : 'aspect-[5/4]';
  const contentClass = compact ? 'p-1 space-y-0.5 min-h-0' : 'p-3 space-y-2';
      ...
      <Link to={`/product/${product.id}`} className="block">
        <div className={`relative ${imageClass} bg-brown-light overflow-hidden`}>
          ...
          <div className="absolute inset-x-1.5 top-1.5 flex items-center justify-between gap-1">
            ...
            <button onClick={handleWishlist} className={`rounded-full bg-white/95 flex items-center justify-center shadow-sm border border-[#D9B48A]/30 ${compact ? 'w-4 h-4' : 'w-9 h-9'}`}>
              <Heart size={compact ? 7 : 15} className={wishlisted ? 'fill-[#A15B2A] text-[#A15B2A]' : 'text-[#5E3A25]'} />
            </button>
          </div>
          <button onClick={handleAdd} className={`absolute bottom-1 right-1 rounded-full gradient-primary flex items-center justify-center shadow-md transition-opacity ${compact ? 'w-4 h-4 opacity-100' : 'w-10 h-10 opacity-0 group-hover:opacity-100'}`}>
            <Plus size={compact ? 8 : 18} className="text-white" />
          </button>
        </div>
        <div className={contentClass}>
          <div className={`flex items-center justify-between gap-1 ${compact ? 'gap-0.5' : ''}`}>
            <p className={`font-semibold uppercase tracking-[0.1em] text-[#6F5745] ${compact ? 'text-[5px]' : 'text-[11px]'}`}>{product.category || 'General'}</p>
            <span className={`product-label ${product.free_shipping ? 'bg-[#22C55E]/12 text-[#166534]' : 'bg-[#E4C9A6]/20 text-[#6F4E2A]'} ${compact ? 'text-[5px] px-0.5 py-0.2' : ''}`}>
              {product.country_of_origin || 'Kenya'}
            </span>
          </div>
          <div>
            <h3 className={`font-display font-semibold text-[#2F241E] line-clamp-2 leading-tight ${compact ? 'text-[7px]' : 'text-base'}`}>{product.name}</h3>
          ...
          <div className={`flex items-center justify-between gap-1 ${compact ? 'gap-1' : ''}`}>
            <div>
              <div className={`flex items-center gap-1 ${compact ? 'mb-0' : 'mb-1'}`}>
                <Star size={compact ? 6 : 12} className="fill-[#F59E0B] text-[#F59E0B]" />
                <span className={`font-semibold text-[#6F4E2A] ${compact ? 'text-[6px]' : 'text-xs'}`}>{product.rating > 0 ? product.rating.toFixed(1) : '4.5'}</span>
              </div>
              <div className={`flex items-center gap-1 text-[#6F4E2A] ${compact ? 'text-[7px]' : 'text-[13px]'}`}>
                <span className="price-display">{formatPrice(product.price, product.currency)}</span>
                {product.old_price && <span className={`text-[#9A7B5B] line-through ${compact ? 'text-[5px]' : 'text-xs'}`}>{formatPrice(product.old_price, product.currency)}</span>}
              </div>
            </div>
